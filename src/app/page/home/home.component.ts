...
export class HomeComponent implements OnInit, OnDestroy {
  ...
  private verticalFocusTravel(targetItem: HTMLElement,down:boolean = false){
    ...
    while(count<10000){ // while true could and did go to infinity
      ...
      if(down && width>=parentWidth && (nextLeft>=targetLeft*0.9 || nextRight>=targetRight*0.9)) break;
      if(!down && width<=parentLeft+100  && (nextLeft<=targetLeft*1.1 || nextRight<=targetRight*1.1)) break;        
    }
